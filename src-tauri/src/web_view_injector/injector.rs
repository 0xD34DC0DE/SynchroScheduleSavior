...
use crate::web_view_injector::inter_webview_promise::{InterWebviewPromiseHandles};
...
trait WindowInjectorExtInternal<R: Runtime> {
    fn inject(&self, promise_handles: &InterWebviewPromiseHandles, injection_args: InjectionArgs) -> Result<()>;
}

impl<R: Runtime> WindowInjectorExtInternal<R> for Window<R> {
    fn inject(&self, promise_handles: &InterWebviewPromiseHandles, injection_args: InjectionArgs) -> Result<()> {
        let formatted_injection = format_injection(promise_handles, injection_args)?;
...
impl<R: Runtime> WindowInjectorExt<R> for Window<R> {
    fn inject_into(&self, target: &Window<R>, injection_args: InjectionArgs) -> Result<&InterWebviewPromiseHandles> {
        ...
        target.inject(handles, injection_args)?;
        Ok(handles)
    }
}

fn format_injection(promise_handles: &InterWebviewPromiseHandles, injection_args: InjectionArgs) -> Result<String> {
    ...
    Ok(
        format!("window.{}('{}', '{}', {}, {});",
                INJECTOR_VAR_NAME,
                promise_handles.result,
                promise_handles.cancel,
